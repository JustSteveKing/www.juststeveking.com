...
import type { de } from '@nuxt/ui-pro/runtime/locale/index.js'
...
<template>
    <UPage v-if="page">
        <UPageHero :title="page.title" :description="page.description" orientation="horizontal" :ui="{
            ...
            <template #links>
                <NuxtLink v-for="(link, index) in page.links" :key="index" :to="link.url" class="u-link u-link--primary">
                    {{ link.title }}
                </NuxtLink>
            </template>
        </UPageHero>

        <UPageSection :title="page.plans.title" :description="page.plans.description" :ui="{
            container: '!p-0',
            title: '!mx-0 text-left',
            description: '!mx-0 text-left',
        }">
           <UPageList>
            <UPageCard v-for="(plan, index) in page.plans.items" :key="index" variant="ghost">
                <template #leading>
                    <h3>{{ plan.title}}</h3>
                </template>
                <template #title>
                ...
                <template #footer>
                    <div class="space-y-6">
                        ...
                        <ol class="list-disc pl-5">
                            <li v-for="(include, index) in plan.details.includes" :key="index">
                                {{ include }}
                            </li>
                        </ol>
                    </div>
                    <div v-if="plan.details.excludes">
                        <h4>Excludes</h4>
                        <ol class="list-disc pl-5">
                            <li v-for="(exclude, index) in plan.details.excludes" :key="index">
                                {{ exclude }}
                            </li>
                        </ol>
                    </div>
                    <div v-if="plan.details.deliverables">
                        <h4>Deliverables</h4>
                        <ol class="list-disc pl-5">
                            <li v-for="(deliverable, index) in plan.details.deliverables" :key="index">
                                {{ deliverable }}
                            </li>
                        </ol>
                    </div>
                    <div v-if="plan.details.terms">
                        <h4>Terms</h4>
                        <ol class="list-disc pl-5">
                            <li v-for="(term, index) in plan.details.terms" :key="index">
                                {{ term }}
                            </li>
                        </ol>
                    </div>
        ...
        >
            <template #links>
                <UButton to="mailto:juststeveking@proton.me" target="_blank">
                    Email me
                </UButton>
            </template>
            <ol class="list-disc pl-5">
            ...
            <UAlert type="info" title="Individual Support" description="For developers who want to support my work, GitHub Sponsors, YouTube memberships, and direct contributions are always appreciated and help keep this content free for everyone." />
